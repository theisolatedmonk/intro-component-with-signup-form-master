...
export function FormElement(props: FormType) {
    return (
        <div className="flex flex-col text-[hsl(249,10%,26%)] gap-1">
            <div className="flex border-gray-500 border p-2 rounded-lg justify-between">
                <input type="text" placeholder={props.placeholder} className='w-full' />
                <Image src={alertIcon} alt={''} />
            </div>
            <p className="text-[hsl(0,100%,74%)] text-right">{props.alert}</p>
        </div>
...
export default function Form({ }: Props) {
    return (
        <div className='flex flex-col bg-white   rounded-lg p-4 gap-4'>
            ...
            <FormElement alert={'Look like this is not an email '} placeholder={'email@example/com'} />
            <FormElement alert={'Password cannot be empty'} placeholder={'Password'} />

            <button className='bg-[hsl(154,59%,51%)] p-2 rounded-md'>CLAIM YOUR FREE TRAIL</button>
            <p className="text-[hsl(246,25%,77%)]">By clicking th e button, you are agreeing to our <span className='text-[hsl(0,100%,74%)]'>Terms and Services</span></p>
        </div>
    )
}